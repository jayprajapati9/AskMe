...
<body>
    ...
    <div class="w-10/12 mx-auto py-3.5">
        <div class="w-full max-w-2xl mx-auto flex items-center py-1 px-1 border-[1.5px] rounded">
            <input type="text" class="w-full outline-none font-normal text-sm" placeholder="Type Username..." id="UserNameInput" maxlength="20">
            <i class="bi bi-search text-blue-500 px-3 cursor-pointer" id="SearchBtn"></i>
        </div>
        <p class="w-full max-w-2xl mx-auto text-red-400 text-[14px] pl-1 pt-1" id="SearchErr"></p>
    </div>
    ...
    <script>
        document.getElementById("SearchBtn").addEventListener("click", function() {
            ...
            if (usernameInput != "") {
                SearchUser(usernameInput);
            } else {
                EmptySearchWrap();
                console.log("Please enter a username");
                $("#SearchErr").text("Kuch toh enter kar bhay");
                setTimeout(() => {
                    $("#SearchErr").text(null);
                }, 5000);
            }
